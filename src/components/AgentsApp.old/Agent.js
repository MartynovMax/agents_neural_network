...
export default class Agent {
  ...
  init () {
    ...
    this._fixture = $body.createFixture(shape, fixtureDef, 1);
    ...
    setInterval(() => {
      ...
      console.log('setInterval: ', this._fixture.render)
      this._fixture.render = {
        fill: i % 2 === 0 ? '#0077ff' : 'red',
        stroke: '#ffffff'
      }
      // console.log('fill: : ', i, i % 2 === 0 ? '#0077ff' : 'red')
    }, 1000)
  }


  applyForce (force) {
    // var f = this.$body.getWorldVector(Vec2(0.0, force))
    // var p = this.$body.getWorldPoint(Vec2(0.0, force))
    var f = this.$body.getWorldVector(Vec2(force, 0))
    var p = this.$body.getWorldPoint(Vec2(force, 0))
